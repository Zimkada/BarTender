-- MIGRATION: Ajouter paramètre p_validated_by à create_sale_with_promotions
-- DATE: 2026-01-09
-- OBJECTIF: Permettre au gérant de spécifier qui valide la vente en mode simplifié
...
-- =====================================================
-- ÉTAPE 2: Créer la nouvelle version avec p_validated_by
-- =====================================================
CREATE OR REPLACE FUNCTION public.create_sale_with_promotions(
    ...
    p_validated_by UUID DEFAULT NULL,  -- ✨ NOUVEAU PARAMÈTRE
...
BEGIN
    ...
    -- ✨ LOGIQUE CORRIGÉE: Utiliser p_validated_by si fourni, sinon fallback sur p_sold_by
    -- Cela permet au gérant de spécifier qui valide en mode simplifié
    -- Tout en maintenant la rétrocompatibilité avec le code existant
    INSERT INTO public.sales (
        ...
        CASE WHEN p_status = 'validated' THEN COALESCE(p_validated_by, p_sold_by) ELSE NULL END,  -- ✨ CORRECTION ICI
...
-- =====================================================
-- ÉTAPE 4: Commentaire explicatif
-- =====================================================
COMMENT ON FUNCTION public.create_sale_with_promotions IS 
'Create a sale with promotion handling. 
New parameter p_validated_by allows specifying who validates the sale (useful in simplified mode where manager assigns sales to servers).
If p_validated_by is NULL, falls back to p_sold_by for backward compatibility.';
