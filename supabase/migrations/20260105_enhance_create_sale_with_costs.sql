-- MIGRATION: Améliorer create_sale_with_promotions pour inclure les coûts de produits
-- DATE: 2026-01-05
-- OBJECTIF: Remplir automatiquement product_cost_unit et product_cost_total lors de la création de promotions

-- =====================================================
-- ÉTAPE 1: Déclarer la fonction améliorée
-- =====================================================
...
BEGIN
    ...
    LOOP
        ...
        IF v_promotion_id IS NOT NULL AND v_discount_amount > 0 THEN
            BEGIN
                -- ✨ NOUVEAU: Récupérer le coût actuel du produit
                v_product_cost_unit := COALESCE(
                    (SELECT COALESCE(bp.current_average_cost, 0)
                     FROM public.bar_products bp
                     WHERE bp.product_id = v_product_id AND bp.bar_id = p_bar_id),
                    0
                );

                -- Créer l'enregistrement promotion_applications avec les coûts
                INSERT INTO public.promotion_applications (
                    ...
                    product_cost_unit, product_cost_total,
                    ...
                    v_product_cost_unit, v_product_cost_unit * v_quantity,
