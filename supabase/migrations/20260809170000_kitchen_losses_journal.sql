...
DECLARE
  ...
  v_end     DATE;
  v_close   INTEGER;
  -- ⭐ Un SEUL enregistrement porte les lignes ET les totaux : les CTE d'un
  -- `WITH` ne survivent pas à la requête qui les suit (cf. correction plus bas).
  v_totals  RECORD;
BEGIN
   ...
   * La version précédente enchaînait deux `SELECT ... INTO` : le second était
   * une requête SÉPARÉE et ne voyait plus ni `toutes` ni `bornees`.
   *
   * ⚠️ LA CRÉATION RÉUSSISSAIT QUAND MÊME. PL/pgSQL ne résout ses requêtes
   * qu'À L'EXÉCUTION : les cinq contrôles du post-vol sont tous passés au vert
   * sur une fonction cassée. Ils lisent la signature, les privilèges et le
   * TEXTE de la définition - jamais un résultat.
   * ⛔ Et le smoke test s'arrête sur la garde `is_bar_member` avant d'atteindre
   * la requête : il valide l'isolation, pas le corps.
   *
   * ⭐ Les deux agrégats sont désormais calculés en UN SEUL passage :
   * `lines` par une sous-requête sur `bornees`, les totaux sur `toutes`.
   */
  SELECT
    COALESCE((
      SELECT jsonb_agg(
    ...
    -- ⭐ Les TOTAUX portent sur `toutes` : ils couvrent la période ENTIÈRE
    -- même quand le détail est tronqué, sinon le chiffre de tête mentirait.
