...
BEGIN
  ...
  SELECT
    COALESCE(jsonb_agg(
      jsonb_build_object(
        ...
        -- ⚠️ NULL sur les gestes antérieurs au 09/08 - un auteur ne s'invente
        --    pas rétroactivement.
        'actor_name',  u.name
      )
  ...
  /**
   * ⭐ TOTAUX PAR SOURCE, calculés sur la MÊME base que les lignes.
   * ⚠️ Les recalculer autrement ferait diverger le total de son détail - le
   * promoteur verrait une somme qui ne correspond pas à ce qu'il additionne.
   */
  SELECT
    COALESCE(SUM(value), 0)                                        AS total_value,
    COUNT(*)                                                        AS total_count,
    COALESCE(SUM(value) FILTER (WHERE source = 'dish'), 0)         AS dish_value,
