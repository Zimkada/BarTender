-- =====================================================
-- RESTAURATION du garde-fou prix STRICT sur create_sale_idempotent
-- =====================================================
-- Date : 2026-07-04
--
-- CONTEXTE : la migration 20260704063000 avait assoupli le garde-fou prix
-- (condition b) pour tolérer un discount NÉGATIF (majoration), afin de faire
-- passer les promos 'majoration_produit'. Mais la fonctionnalité majoration a
-- été RETIRÉE du front (commit 8437b87 — jamais fonctionnelle, le moteur
-- calculateBestPrice ne retient que ce qui baisse le prix). Une majoration de
-- prix sera une fonctionnalité DÉDIÉE et SÉPARÉE, qui n'utilisera PAS ce
-- chemin de vente.
--
-- Donc la tolérance négative n'a plus aucun usage légitime — mais elle laisse
-- une petite surface de fraude (un client modifié pourrait envoyer un
-- discount_amount négatif → vente à prix majoré frauduleux). On RESTAURE le
-- garde-fou strict (discount négatif interdit), état d'origine de la Vague 4e.
--
-- CERTIFICATION AVANT ÉCRITURE :
--   - Corps ci-dessous = pg_get_functiondef EXACT de la prod (2026-07-04),
--     copié tel quel. nb_versions=1 et grantees={authenticated,postgres,
--     service_role} (pas de public/anon) vérifiés AVANT.
--   - SEULES 2 LIGNES changent vs le corps prod actuel :
--       (b) borne [-max, max] → « < 0 OR > max » (négatif de nouveau interdit)
--       insertion promo : « <> 0 » → « > 0 » (plus de majoration à tracer)
--   - REVOKE PUBLIC + GRANT rétabli en fin (un CREATE OR REPLACE dans cette
--     base perd les grants → proacl NULL = brèche anon).
-- =====================================================
