-- =====================================================
-- Migration: Fix create_sale_idempotent security check
-- Date: 2026-02-10
-- Objectif: Supprimer le check super_admin inutile qui référence une colonne inexistante
-- =====================================================

-- Supprimer les anciennes versions
DROP FUNCTION IF EXISTS public.create_sale_idempotent(
    uuid, jsonb, text, uuid, text, uuid, text, text, text, text, date, uuid
);
...
-- Recréer la fonction sans le check super_admin
CREATE OR REPLACE FUNCTION public.create_sale_idempotent(
    ...
    p_source_return_id UUID DEFAULT NULL -- ✨ Pour la traçabilité échange
)
RETURNS sales
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    ...
    -- 🛡️ SECURITY CHECK (simplifié - sans super_admin)
    IF NOT EXISTS (
