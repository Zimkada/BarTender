...
- Ou le RPC vérifie un **claim signé et borné dans le temps** (un JWT applicatif comme en option A, mais cette fois le RPC vérifie son contenu plutôt que de faire confiance au paramètre), ce qui ramène en réalité à devoir résoudre les mêmes questions que l'option A.
...
**Conséquence pour le séquencement (§10) : ni A ni B ne peut être qualifiée de "recommandée" tant que l'une des deux n'a pas été concrètement testée.** L'étape 2 du séquencement doit être reformulée en "démontrer techniquement un mécanisme d'appel sécurisé (A ou B) sur un unique RPC pilote, avant d'écrire quoi que ce soit d'autre" — pas "choisir entre A et B" comme si les deux étaient déjà des options implémentables.
...
| Un RPC existant peut-il être exempté de son guard pour ce nouveau besoin ? | Non — option A (session applicative) préférée ; sinon RPC dédiés séparés, jamais de modification en place (§6) |
...
| Le JWT applicatif de l'option A (§6) peut-il hériter du mécanisme d'impersonation existant ? | Risque réel si le chemin de génération est partagé ou mal isolé — contournerait silencieusement 15+ policies RLS, pas seulement le RPC ciblé (§6) |
...
- **Le mécanisme d'appel sécurisé des RPC (option A ou B du §6) n'est PAS encore démontré techniquement** — ni l'un ni l'autre n'est une décision actionnable en l'état ; c'est le prochain test concret à faire, pas un choix de préférence entre deux options également prêtes.
...
2. **Démontrer concrètement**, par un test isolé documenté, un mécanisme d'appel RPC sécurisé (option A ou B du §6) — aucune des deux n'est une décision actionnable tant qu'elle n'a pas été vérifiée en pratique, pas seulement décrite. Ce test se fait sur un unique RPC pilote (`get_bar_admin_stats` par exemple).
