...
Deux pistes, aucune encore validée techniquement, à ne jamais mélanger dans un même RPC :

**Piste Session — ne pas toucher aux RPC existants — hypothèse à démontrer avant tout code.** L'idée : l'Edge Function, après avoir résolu l'identité (§4) et validé les droits, appellerait les RPC **sous une session utilisateur réelle** plutôt que sous `service_role` brut, via un token généré pour l'occasion au nom du promoteur résolu. Le guard `is_bar_member(p_bar_id)` fonctionnerait alors tel quel, sans aucune modification de RPC. **Cette option n'a pas été vérifiée dans cette étude** — ni la faisabilité technique précise (quel appel Supabase génère un tel token depuis une Edge Function, sous quel rôle admin), ni ses paramètres de sécurité (claims exacts portés par le token, durée de validité, mécanisme de révocation, valeur de `aud`/`sub`/`role`, garantie d'absence du claim d'impersonation — voir l'alerte ci-dessous). **Avant de la retenir, il faut : (1) prouver le mécanisme exact par un test isolé sur un projet Supabase de dev, en documentant précisément la commande/API utilisée et le contenu du token obtenu ; (2) si le mécanisme n'existe pas ou est trop fragile, l'abandonner franchement plutôt que de la garder comme "option préférée" non démontrée.**

**Précision factuelle du 20/08/2026** : le repo contient un précédent d'usage de l'API Admin Supabase depuis une Edge Function (`supabase/functions/create-bar-member/index.ts`, `adminClient.auth.admin.createUser(...)`), mais il ne démontre **pas** le mécanisme dont la piste Session a besoin. Ce code **crée** un nouvel utilisateur ; il ne **génère pas de session/token au nom d'un utilisateur déjà existant** pour appeler des RPC en son nom, ce qui est une capacité distincte de l'API Admin. Ce précédent confirme seulement que le projet manipule déjà `service_role` + API Admin depuis une Edge Function — il ne lève pas l'incertitude sur la piste Session elle-même, qui reste entièrement à démontrer.

> **⚠️ Précaution obligatoire, valable dès lors que la piste Session serait confirmée.** Un mécanisme d'impersonation existe déjà et conditionne plus de 15 policies RLS (`bars`, `bar_members`, `bar_products`, `sales`, `expenses`, `salaries`, `accounting_transactions`, etc. — `20251213_enable_rls_bypass_for_impersonation.sql`) : `is_impersonating()` lit `auth.jwt()->'user_metadata'->>'impersonation' = 'true'` et, si vrai, **contourne la RLS de toutes ces tables**, pas seulement celle ciblée par un appel. Le token applicatif généré pour l'agent WhatsApp ne doit **jamais**, sous aucun prétexte, porter ce claim. À vérifier explicitement dans le test de faisabilité lui-même (point 1 ci-dessus), pas après coup : inspecter le contenu du token généré et confirmer l'absence de ce claim avant de considérer la piste validée.
...
**Conséquence pour le séquencement (§10) : ni la piste Session ni la piste RPC dédiés ne peut être qualifiée de "recommandée" tant que l'une des deux n'a pas été concrètement testée.** L'étape 2 du séquencement doit être reformulée en "démontrer techniquement un mécanisme d'appel sécurisé (Session ou RPC dédiés) sur un unique RPC pilote, avant d'écrire quoi que ce soit d'autre" — pas "choisir entre les deux" comme si elles étaient déjà des options implémentables.
...
| Un RPC existant peut-il être exempté de son guard pour ce nouveau besoin ? | Non, quelle que soit l'option retenue — ni la piste Session (session applicative) ni la piste RPC dédiés n'est encore démontrée techniquement (§6, corrigé le 19-20/08/2026) ; jamais de modification en place des RPC existants dans les deux cas |
...
| Le JWT applicatif de la piste Session (§6) peut-il hériter du mécanisme d'impersonation existant ? | Risque réel si le chemin de génération est partagé ou mal isolé — contournerait silencieusement 15+ policies RLS, pas seulement le RPC ciblé (§6) |
...
- **Le mécanisme d'appel sécurisé des RPC (piste Session ou piste RPC dédiés, §6) n'est PAS encore démontré techniquement** — ni l'une ni l'autre n'est une décision actionnable en l'état ; c'est le prochain test concret à faire, pas un choix de préférence entre deux options également prêtes.
...
Ordre de travail recommandé, chaque étape validée avant la suivante (révisé le 19/08/2026 suite à une revue externe — l'étape 2 n'est plus "choisir" mais "démontrer", et l'étape 1 inclut désormais explicitement la refonte de `wa_conversations`) :

1. Concevoir et faire certifier le schéma du lien numéro→bar avec sa contrainte composite (§4), **et** la migration de `wa_conversations` pour supporter le multi-bar (§4bis), avec le flux d'opt-in — **avant** d'écrire un seul tool.
2. **Démontrer concrètement**, par un test isolé documenté, un mécanisme d'appel RPC sécurisé (piste Session ou piste RPC dédiés, §6) — aucune des deux n'est une décision actionnable tant qu'elle n'a pas été vérifiée en pratique, pas seulement décrite. Ce test se fait sur un unique RPC pilote (`get_bar_admin_stats` par exemple).
...
Ne jamais sauter l'étape 1 pour aller plus vite sur les tools — c'est l'inverse de la priorité réelle de ce projet.
