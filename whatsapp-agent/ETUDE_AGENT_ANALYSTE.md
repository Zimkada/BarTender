...
### 4ter. Le lien n'est pas un aiguillage de mode — c'est une autorisation de tools (ajouté le 21/08/2026)

**Question soulevée en préparant l'étape 1 : un numéro lié à un bar continue de poser des questions de type "comment créer un produit ?" (support/guide, le rôle actuel d'Aïcha) aussi souvent que "combien j'ai vendu cette semaine ?" (données réelles). "Numéro lié → mode analyste" est donc une simplification fausse — le lien ne peut pas être un aiguillage binaire par conversation.**

Deux couches distinctes, à ne jamais fusionner :

1. **Autorisation (résolue par le code, une fois par message, avant l'appel à Claude — inchangé par rapport au §3/§4)** : ce numéro est-il lié à un bar, et le rôle résolu a-t-il `canViewAnalytics` ? Répond `bar_id` ou rien.
2. **Intention (laissée au modèle, comme aujourd'hui)** : cette question précise porte-t-elle sur le fonctionnement de l'app ou sur des données réelles ? Ce n'est **pas** une décision de routage à prendre en code — c'est exactement le mécanisme de tool-use déjà en place (Claude choisit déjà entre `enregistrer_lead`, `escalader_humain`, ou une simple réponse en langage naturel selon la question posée).

**Précision trouvée en code review (21/08/2026), pour lever une tension apparente avec la migration `wa_conversations` (§4bis) :** "résolu une fois par message" (couche 1, ci-dessus) ne veut pas dire "une seule conversation qui bascule dynamiquement de bar en bar". `wa_conversations.bar_id` reste par ligne de conversation — un numéro multi-bar a une ligne de conversation par bar lié (§4bis), chacune avec son propre historique. La couche 2 (intention) se résout *à l'intérieur* d'une conversation déjà rattachée à un bar donné, pas en changeant de bar au fil des messages d'un même thread. Le §9 ("gestion précise du cas multi-bar dans le prompt et le flux conversationnel") reste donc entièrement ouvert pour un point précis : comment le webhook choisit *quelle* ligne `wa_conversations` charger quand un numéro a plusieurs bars liés (le message identifie-t-il le bar visé ? Faut-il le demander explicitement ? Cf. §8, "un promoteur multi-bar : quel bar la question concerne-t-elle").

**Conséquence pour l'implémentation** : il n'y a **pas** de "mode analyste" séparé au sens d'un branchement de code qui changerait de modèle ou de prompt entier. À la place, l'ensemble de tools passé à l'appel Claude est composé dynamiquement par le code, une fois l'autorisation résolue :
- Numéro non lié, ou lié mais rôle sans `canViewAnalytics` : liste de tools = celle d'aujourd'hui (support/guide, `enregistrer_lead`, `escalader_humain`, `definir_profil`). **Les tools de données réelles ne sont même pas présents dans le schéma envoyé à Claude** — pas de garde applicative après coup à faire confiance, le modèle n'a structurellement pas la possibilité de les appeler (cohérent avec §5, "filtré à la résolution d'identité, jamais en espérant que le prompt refuse poliment").
- Numéro lié avec rôle habilité : liste de tools = celle d'aujourd'hui **plus** les tools de données réelles du §5 (`obtenir_resume_ventes`, `obtenir_top_produits`, etc.). Claude décide, par question, lequel appeler — ou n'en appelle aucun si la question est une question de support ordinaire.

**Conséquence pour le choix du modèle (révise partiellement §7) :** le routage Sonnet/Haiku ne peut plus se faire "par mode" au sens d'un mode figé pour toute la conversation, puisqu'un même numéro lié peut alterner entre les deux types de questions d'un message à l'autre. Il se fait par **conversation résolue** : dès qu'un numéro est lié avec le rôle habilité, ses messages passent systématiquement par Sonnet (nuance nécessaire même pour les questions de support, et bascule de modèle en cours de conversation casserait le cache système — optimisation n°1 du §7bis). Les numéros non liés restent sur Haiku, comme aujourd'hui. Le prompt système suit la même logique : assemblé conditionnellement (le bloc "tools données réelles + règles anti-invention chiffrées" du §7 n'est inclus dans le prompt caché que pour les conversations liées), pour ne pas alourdir le prompt caché de la majorité des messages (prospects non liés).
...
|---|---|---|
| `obtenir_resume_ventes(periode)` | ~~`get_bar_admin_stats`~~ **`get_bar_daily_stats`** (20260823100000, voir §10) ⚠️ | CA, nombre de ventes, tendance vs période précédente |
...
**`get_top_products_aggregated`** a été retouchée le 05/08/2026 (`20260805100000_top_products_include_dishes.sql`) suite à un signalement terrain : la version antérieure groupait tous les plats (qui n'ont pas de `product_id`, seulement `item_type: 'dish'` + `dish_id`) en une seule ligne fusionnée, et affichait une **marge de 100 % sur tous les plats** (`COALESCE(cump, 0)` où `cump` vient de `bar_products.current_average_cost`, table où un plat n'existe pas).

**⚠️ CORRECTION (23/08/2026, lecture directe du code avant d'écrire le 2e tool) : les DEUX défauts sont en fait corrigés, pas seulement le premier comme l'affirmait cette section jusqu'ici.** Le corps réel du RPC (relu ligne par ligne) calcule le coût des plats via un CTE dédié `dish_costs` (`SUM(kitchen_order_items.computed_cost)`, pré-agrégé par `(sale_id, dish_id)` pour éviter le fanout) et bascule le calcul de `profit` selon la nature de l'article : `CASE WHEN ap.dish_cost_total IS NOT NULL THEN (total_revenue - dish_cost_total) ELSE (total_revenue - quantity*cump) END` — jamais le CUMP pour un plat. Certifié en prod le 05/08 (post-vol collé dans la migration : 3 calculs indépendants convergent sur le même écart de marge d'un plat de test). Le RPC prend déjà `p_start_date`/`p_end_date` (filtre sur `business_date`, même discipline que `get_bar_daily_stats`). **Conséquence pour le tool `obtenir_top_produits`** : il peut restituer `profit` tel quel pour toute ligne (boisson ou plat), aucun contournement ni croisement avec `get_kitchen_metrics` n'est nécessaire.
...
**⚠️ MISE À JOUR (23/08/2026, test terrain + code review) : `get_bar_admin_stats` n'est PLUS le RPC utilisé par le tool `obtenir_stats_bar`** — remplacé par `get_bar_daily_stats` (migration `20260823100000`, voir §10) suite à un bug réel trouvé au premier test réel : `get_bar_admin_stats` cumule TOUT l'historique du bar (aucun filtre de date), un promoteur demandant "aujourd'hui" recevait un total depuis toujours. Le nouveau RPC filtre `total_sales`/`total_revenue`/`pending_sales` par `business_date` (paramètre obligatoire, jamais calculé côté SQL — même règle que `get_daily_scope_totals`, §5). `total_products` reste non daté dans les deux RPC (limite ci-dessus inchangée) — le tool le signale désormais dans un sous-objet structurellement séparé des champs datés, pas seulement en prose.
...
> **IMPLÉMENTÉ le 24/08/2026** (§10 étape 5, close) : table dédiée `wa_analyst_tool_audit`, en production. Le choix de la table dédiée plutôt que de `wa_conversations` étendu, le format exact des colonnes, les décisions de conception et les quatre défauts trouvés en revue avant exécution sont détaillés en §9.
...
| Un promoteur multi-bar : quel bar la question concerne-t-elle ? | **Tranché et implémenté le 21/08/2026** : un seul bar "actif" à la fois par numéro (`wa_bar_links.is_active_link`, contrainte d'unicité en base). Le premier lien vérifié devient actif automatiquement (cas mono-bar, majoritaire, zéro étape supplémentaire) ; passer à un autre bar exige un choix explicite du promoteur, jamais un choix silencieux du premier trouvé, via `change_active_wa_bar_link()`. Reste à spécifier : la formulation exacte du prompt/flux conversationnel qui déclenche cette bascule (ex. quelle phrase du promoteur appelle la fonction) — c'est un détail d'UX conversationnelle, pas un angle mort de sécurité. |
...
- ~~Le flux exact de l'opt-in (option A, §4) côté application~~ — **FLUX COMPLET DE BOUT EN BOUT au 23/08/2026** : 5 migrations DB exécutées et vérifiées en prod (20260822090000 request_wa_bar_link+verify_wa_bar_link_code, 20260822090001 resolve_wa_bar_link allowlist, 20260822090002 dedup wamid, 20260822090003 ignore demandes inertes cross-bar, 20260823090000 get_wa_bar_link_status), Edge Functions `request-wa-bar-link` et `wa-webhook` (étendue) déployées en prod (non-régression du bot commercial confirmée), UI `WaBarLinkSection` écrite dans `SettingsPage` (onglet Fonctionnement). Bug bloquant trouvé en code review multi-angle avant tout test réel : le service front contournait le RPC `get_wa_bar_link_status` fraîchement écrit (lecture directe de la table verrouillée service_role au lieu d'appeler le RPC) — corrigé. **Second bug réel trouvé au premier test terrain (capture d'écran) : `normalizePhoneToMetaFormat` ne gérait pas le plan de numérotation béninois 2021 (préfixe `01` ajouté à tous les numéros existants) — `0197548310` produisait `229197548310` (12 chiffres, `1` de trop) au lieu de `22997548310` (11 chiffres, format confirmé par 5 échantillons réels de `wa_conversations.phone`). Corrigé : `01` retiré en entier avant préfixage. Redéployé.** Décisions prises (avant écriture du code) :
  ...
  - **Isolation stricte du canal commercial** : le webhook intercepte un message de 6 chiffres **avant** de charger `wa_conversations` — si une demande en attente correspond, il la traite entièrement et retourne, sans jamais créer ni modifier une ligne de conversation commerciale pour ce numéro.
  - **Emplacement UI** : nouvelle section dans `SettingsPage` (pas de route dédiée), cohérent avec le reste des réglages du bar, gardé par `canManageSettings`.
  - **Confirmation** : uniquement via une réponse WhatsApp au code reçu — jamais un champ de ressaisie côté app, qui viderait la preuve de possession du numéro de sa valeur (l'app affiche juste l'état "en attente" / "confirmé", sans jamais revalider le code elle-même).
...
- ~~Le format exact de journalisation d'audit (§7)~~ — **tranché, exécuté en prod et déployé le 24/08/2026** (migration `20260824090000_create_wa_analyst_tool_audit.sql`, post-vol certifié sur 11 points). Table dédiée `wa_analyst_tool_audit` : `bar_id`, `user_id`, `phone_wa_id`, `role` (revalidé à cet appel, jamais le `role_snapshot` figé), `tool_name`, `tool_input` (JSONB), `success`, `error_message`, `duration_ms`, `work_ms`, `created_at`. Décisions et leurs raisons :
  - **Table dédiée plutôt que `audit_logs`** (générique existante) : ses `NOT NULL` sur `user_name`/`user_role`/`description` obligeraient le webhook à joindre `users` + `bar_members` à chaque tool juste pour remplir un libellé et à inventer une phrase narrative par tool (c'est un journal machine, pas un événement métier raconté), et son `CHECK` fermé sur `related_entity_type` n'a aucune valeur adaptée à un appel de tool — l'y forcer casserait la contrainte ou obligerait à modifier une table partagée avec l'app web pour un besoin qui lui est étranger.
  - **Aucune FK sur `bar_id`/`user_id`** : un journal d'audit ne doit jamais perdre de lignes parce que le bar ou le compte a été supprimé — c'est précisément le cas où on a besoin de la trace. (`wa_bar_links` porte des FK `ON DELETE CASCADE` car c'est une table d'état courant, pas d'historique.)
  - **Verrouillage `service_role` seul** (RLS active sans policy + REVOKE `anon`/`authenticated`), `GRANT SELECT, INSERT` uniquement : append-only garanti au niveau des privilèges, le webhook ne peut pas modifier ni supprimer une ligne écrite.
  - **Seuls les tools analystes sont journalisés**, via le test sur `analystLink` plutôt qu'une liste de noms de tools : `activeTools` vaut `ANALYST_TOOLS` si et seulement si `analystLink` est résolu, donc un futur 6e tool est couvert d'office, sans liste parallèle à maintenir. Les tools commerciaux (Aïcha) restent hors du journal — aucune donnée sensible en jeu, et les y mêler brouillerait la séparation des deux canaux.
  - **Deux mesures de durée distinctes** : `duration_ms` (latence réellement subie par le promoteur, cérémonie de session Auth comprise) et `work_ms` (travail Postgres seul). Un chiffre unique les confondrait, rendant un bar à fort volume indiscernable d'une session Auth lente — ce qui priverait le §7bis de la mesure de coût RPC qu'il demande. L'écart entre les deux donne directement le coût de la cérémonie de session.
  - **Écriture non-bloquante** : toute erreur de journalisation est avalée après trace console. Un incident de journalisation ne doit jamais priver le promoteur de sa réponse — le journal est un filet de détection a posteriori, pas un maillon du chemin de réponse.

  **Quatre défauts trouvés en code review (3 passes) avant exécution, dont un critique** : le `GRANT ... TO service_role` manquait. `service_role` contourne RLS mais **pas** les privilèges de table — chaque `INSERT` aurait échoué en `42501`, silencieusement avalé par le helper non-bloquant : le journal serait resté vide indéfiniment alors que tout semblait fonctionner, et le post-vol serait passé au vert (il n'assertait alors que les accès *refusés*, jamais l'accès *requis*). Piège déjà documenté noir sur blanc dans ce projet (`20260719000000_create_whatsapp_agent_tables.sql:150-153`) et déjà appliqué sur `wa_bar_links` — la première version de ce fichier affirmait pourtant "verrouillée comme wa_bar_links" en omettant exactement le GRANT qui la rend fonctionnelle. **Leçon transposable à toute migration future : un post-vol qui n'assère que les accès refusés laisse passer une table verrouillée au point d'être inutilisable.** Les trois autres : `work_ms` journalisant la mesure du tool précédent pour un tool sortant avant d'ouvrir une session (la colonne promet `NULL`, le code livrait un chiffre faux) ; `tool_input` tronqué stocké comme fragment JSON coupé au milieu, inexploitable par la requête de détection cross-bar à laquelle la colonne sert (remplacé par la liste des clés) ; compte d'index affiché mais jamais asserté — le même trou que le GRANT.
...
1. ~~Concevoir et faire certifier le schéma du lien numéro→bar~~ — **schéma exécuté et vérifié en production le 21/08/2026, en 3 migrations** : `wa_bar_links` + `resolve_wa_bar_link()` (privilèges confirmés `service_role` uniquement, `anon`/`authenticated` bloqués sur les deux objets), `wa_conversations.bar_id` + 2 index uniques partiels (post-vol : 8/8 conversations préservées, 0 migrée par erreur, ancienne contrainte `UNIQUE(phone)` bien supprimée), et `is_active_link` + trigger d'auto-activation + `change_active_wa_bar_link()` pour la résolution multi-bar (§8/§9 — décision produit : un seul bar actif à la fois, choisi explicitement par le promoteur, premier lien vérifié auto-actif). **Deux bugs réels trouvés, chacun à un stade différent** : (1) trouvé à l'exécution, non détecté par 8 passes de code review statique — `array_agg(attname)` produit un `name[]`, sans opérateur `=` implicite face à `ARRAY['phone']` (`text[]`), corrigé par cast `attname::text` (commit 2950956) ; (2) trouvé en code review avant toute exécution — la règle produit "premier lien auto-actif" n'était que documentée en commentaire, jamais implémentée dans le schéma (`DEFAULT false` seul), corrigée par un trigger `BEFORE INSERT` + verrouillage consultatif partagé entre trigger et fonction de bascule pour éviter une course concurrente (commit 67f3215). Le flux d'opt-in côté app (UI, code de vérification) reste à concevoir séparément — le schéma est prêt à l'accueillir mais `wa_bar_links` est encore vide, aucun code ne l'alimente.

   **Précision de conception ajoutée en écrivant cette étape (§4ter) :** le lien numéro→bar n'est **pas** un aiguillage de mode par conversation — un promoteur lié continue de poser des questions de support ordinaire (ce qu'Aïcha fait déjà) autant que des questions sur ses données réelles. La résolution d'identité détermine seulement quels *tools* sont exposés à Claude pour ce numéro (autorisation), le choix d'appeler un tool de données ou de répondre en langage naturel reste au modèle (intention), comme pour `enregistrer_lead`/`escalader_humain` aujourd'hui. Voir §4ter pour le détail et l'impact sur le choix de modèle.
...
**Angle mort trouvé le 23/08/2026, en préparant l'étape 3 (avant d'écrire le code)** : `generateLink({ type: 'magiclink', email })` a besoin d'un email, mais `public.users` n'a **aucune colonne email** (auth par `username`, voir `004_custom_auth_complete.sql`) — l'email réel vit dans `auth.users` (Supabase Auth), résolu par `resolve_wa_bar_link()` → `user_id`. De plus, un compte créé par username seul (cas nominal pour un serveur/gérant, pas une exception — confirmé dans `TeamManagementPage.tsx:150,352`) reçoit un email placeholder généré `username@bartender.app`, déjà connu et géré ailleurs dans le repo (`admin_send_password_reset`, `20251215200000...sql:36`, qui distingue explicitement les emails placeholder des emails réels avant d'envoyer un vrai lien). Sans conséquence pour la piste Session : `generateLink` ne dépend jamais du domaine de l'email, seulement de son existence dans `auth.users`, et n'envoie rien (confirmé le 21/08). Décision : résoudre l'email via `adminClient.auth.admin.getUserById(user_id)` directement dans `wa-webhook` (même famille d'appel que `generateLink`/`verifyOtp`, déjà démontrée) plutôt que via un nouveau RPC dédié — pas de nouvelle surface d'exposition pour une donnée qui n'a besoin de sortir de `auth.users` qu'en mémoire de fonction, jamais vers une table publique.
