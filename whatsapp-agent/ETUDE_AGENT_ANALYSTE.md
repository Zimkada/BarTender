...
1. ~~Concevoir et faire certifier le schéma du lien numéro→bar~~ — **schéma exécuté et vérifié en production le 21/08/2026** : `wa_bar_links` + `resolve_wa_bar_link()` (privilèges confirmés `service_role` uniquement, `anon`/`authenticated` bloqués sur les deux objets) et `wa_conversations.bar_id` + 2 index uniques partiels (post-vol : 8/8 conversations préservées, 0 migrée par erreur, ancienne contrainte `UNIQUE(phone)` bien supprimée). **Un bug réel trouvé à l'exécution, non détecté par les 8 passes de code review du 21/08** (aucune n'avait exécuté le SQL réellement) : `array_agg(attname)` sur `pg_attribute` produit un `name[]`, sans opérateur `=` implicite face à `ARRAY['phone']` (`text[]`) — corrigé par cast explicite `attname::text` (commit 2950956), ré-exécuté avec succès. Le flux d'opt-in côté app (UI, code de vérification) reste à concevoir séparément — le schéma est prêt à l'accueillir mais `wa_bar_links` est encore vide, aucun code ne l'alimente.
