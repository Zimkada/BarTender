...
1. Concevoir et faire certifier le schéma du lien numéro→bar avec sa contrainte composite (§4), **et** la migration de `wa_conversations` pour supporter le multi-bar (§4bis), avec le flux d'opt-in — **avant** d'écrire un seul tool. **Schéma écrit le 21/08/2026** : `supabase/migrations/20260821090000_create_wa_bar_links.sql` (table `wa_bar_links` + fonction `resolve_wa_bar_link()`, revalidation `bar_members.is_active` à chaque appel, `service_role` uniquement) et `20260821090001_wa_conversations_multi_bar.sql` (colonne `bar_id` nullable + 2 index uniques partiels, pré-vol/post-vol de non-régression sur les conversations existantes). **Pas encore exécutées en production** — à appliquer manuellement en SQL Editor, jamais `db push` (table `wa_conversations` déjà en prod avec ~1 mois de données réelles). Le flux d'opt-in côté app (UI, code de vérification) reste à concevoir séparément — cette migration ne fait que poser le schéma qui l'accueillera.
