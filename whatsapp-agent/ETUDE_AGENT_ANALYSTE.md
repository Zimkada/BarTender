...
- ~~Le flux exact de l'opt-in (option A, §4) côté application~~ — **conçu le 21/08/2026, migration DB écrite, revue (commits d4c11e5 puis c1f1394) et EXÉCUTÉE EN PRODUCTION le 22/08/2026 (`request_wa_bar_link`, `verify_wa_bar_link_code`, `resolve_wa_bar_link` harmonisée sans `super_admin` — post-vol vérifié par requêtes indépendantes). Reste à écrire : Edge Function `request-wa-bar-link` + extension `wa-webhook` (interception du code à 6 chiffres) + UI dans `SettingsPage`.** Décisions prises (avant écriture du code) :
