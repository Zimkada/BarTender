...
- ~~Le flux exact de l'opt-in (option A, §4) côté application~~ — **conçu le 21/08/2026, migration DB écrite/revue/exécutée le 22/08/2026, Edge Function `request-wa-bar-link` écrite et revue (commit d51007c puis 90c07eb — vrai bug de duplication d'indicatif trouvé en 2e passe), extension `wa-webhook` écrite et revue (commit 5ab88d4 puis 69b595b/6d1dfdd — bug réel de dedup incomplète sur les issues terminales trouvé et corrigé, migrations 20260822090002/20260822090003). Code non encore exécuté en production pour l'Edge Function et l'extension webhook — les migrations SQL associées le sont. Reste à écrire : UI dans `SettingsPage`.** Décisions prises (avant écriture du code) :
