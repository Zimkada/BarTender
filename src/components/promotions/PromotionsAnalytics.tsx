        ...
        <div className="space-y-6 p-5 sm:p-8">
            ...
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h3 className="text-h3 text-foreground">Analyse des promotions</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
                {kpis.map((kpi, index) => (
                    <motion.div
                        ...
                        className="bg-card rounded-2xl p-4 border border-border shadow-sm hover:shadow-md transition-shadow flex items-center gap-3"
                    >
            ...
            <div className="bg-card rounded-2xl border border-border shadow-sm overflow-hidden">
                <div className="px-5 py-4 border-b border-border flex justify-between items-center">
                    <div>
                        <h4 className="text-h3 text-foreground flex items-center gap-2">
                            <Zap className="text-brand-primary" size={16} />
                            Classement des offres
                        </h4>
                        <p className="text-caption text-muted-foreground mt-0.5">Comparaison par rentabilité directe</p>
                    </div>
                </div>
                    ...
                    {stats?.topPromotions?.length > 0 ? (
                        <table className="w-full text-left whitespace-nowrap">
                            <thead className="bg-muted border-b border-border">
                                <tr>
                                    <th className="px-5 py-3 text-micro text-muted-foreground">Promotion</th>
                                    <th className="px-5 py-3 text-micro text-muted-foreground text-center">Score</th>
                                    <th className="px-5 py-3 text-micro text-muted-foreground">CA généré</th>
                                    <th className="px-5 py-3 text-micro text-muted-foreground">Profit net</th>
                                    <th className="px-5 py-3 text-micro text-muted-foreground">Efficacité</th>
                                </tr>
                            </thead>
                                ...
                                {stats.topPromotions.map((promo: any, index: number) => (
                                    <tr key={index} className={`transition-colors ${promo.netProfit < 0 ? 'bg-red-50/50 hover:bg-red-50' : 'hover:bg-muted/50'}`}>
                                        <td className="px-5 py-4">
                                            <div className="flex items-center gap-3">
                                                <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-body-sm font-semibold tabular-nums ${promo.netProfit < 0 ? 'bg-red-100 text-red-600' : 'bg-muted text-muted-foreground'}`}>
                                                    {index + 1}
                                                </div>
                                                <div>
                                                    <div className={`text-body-sm font-semibold ${promo.netProfit < 0 ? 'text-red-600' : 'text-foreground'}`}>{promo.name}</div>
                                        ...
                                        <td className="px-5 py-4 text-center">
                                            <span className={`inline-flex items-center justify-center w-10 h-10 rounded-full border-2 text-body-sm font-semibold tabular-nums ${promo.netProfit < 0 ? 'border-red-200 bg-red-50 text-red-600' : 'border-border text-foreground'}`}>
                                                {promo.roi}%
                                            </span>
                                        </td>
                                        <td className="px-5 py-4">
                                            <div className="text-body-sm font-semibold text-green-600 tabular-nums">{formatPrice(promo.revenue)}</div>
                                            <div className="text-caption text-muted-foreground tabular-nums">Coût : {formatPrice(promo.costOfGoods || 0)}</div>
                                        </td>
                                        <td className="px-5 py-4">
                                            <div className={`text-body-sm font-semibold tabular-nums ${promo.netProfit < 0 ? 'text-red-600' : 'text-foreground'}`}>
                                        ...
                                        <td className="px-5 py-4">
                                            <div className="w-28 h-1.5 bg-muted rounded-full overflow-hidden mb-1">
                                                <motion.div
                                                    initial={{ width: 0 }}
                                            ...
                                            <div className={`text-caption ${promo.netProfit < 0 ? 'text-red-500' : 'text-muted-foreground'} tabular-nums`}>
                        ...
                        <div className="py-16 text-center flex flex-col items-center gap-4">
                            <div className="w-14 h-14 bg-muted rounded-2xl flex items-center justify-center border border-dashed border-border">
                                <BarChart3 size={24} className="text-muted-foreground/60" />
                            </div>
                            <div>
                                <p className="text-body-sm font-medium text-muted-foreground">Aucune donnée pour cette période</p>
                                <p className="text-caption text-muted-foreground mt-1">Lancez une promotion pour voir les performances ici.</p>
                            </div>
                        </div>
                    )}
                </div>
