    ...
    <div className="w-full max-w-2xl mx-auto px-4">
      <div className="bg-white rounded-lg shadow-md p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          >
            + Sélectionner un gérant existant
          </button>

          {/* Shortcut to Team Page */}
          <div className="mt-4 p-4 bg-amber-50 border border-amber-200 rounded-lg flex flex-col sm:flex-row items-center justify-between gap-4">
            <div>
              <h3 className="text-sm font-bold text-amber-900">Besoin de créer un nouveau compte ?</h3>
              <p className="text-xs text-amber-700 mt-1">
                Pour créer de nouveaux profils ou inviter par email, utilisez le menu complet.
              </p>
            </div>
            <button
              type="button"
              onClick={() => {
                // Save any current progress to context before navigating
                updateStepData(OnboardingStep.OWNER_ADD_MANAGERS, formData);
                navigate('/team');
              }}
              className="w-full sm:w-auto px-4 py-2 bg-white border border-amber-300 text-amber-900 text-sm font-medium rounded-lg hover:bg-amber-100 transition shadow-sm whitespace-nowrap"
            >
