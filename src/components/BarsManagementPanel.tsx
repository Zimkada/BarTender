  ...
  return (
    <AnimatePresence>
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={onClose}>
        <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} onClick={(e) => e.stopPropagation()} className="bg-white rounded-2xl shadow-2xl w-full sm:max-w-[calc(100%-32px)] md:max-w-5xl lg:max-w-6xl h-[90vh] overflow-hidden flex flex-col">
          <AdminPanelErrorBoundary fallbackTitle="Erreur dans la gestion des bars">
            <div className="bg-gradient-to-r from-purple-600 to-indigo-600 p-4 md:p-6 text-white relative">
              ...
              <div className="flex items-center gap-3"><Building2 className="w-8 h-8" /><div><h2 className="text-xl md:text-2xl font-bold">Gestion des Bars</h2><p className="text-purple-100 text-sm">Gérer tous les bars de BarTender Pro</p></div></div>
            </div>
