                        ...
                        return (
                            <div key={bar.id} className={`bg-white rounded-lg p-4 border-2 ${bar.isActive ? 'border-green-200' : 'border-red-200'} hover:shadow-lg transition-shadow`}>
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                    ...
                                    <button onClick={() => { const promoteurMember = members.find(m => m.role === 'promoteur'); if (promoteurMember) { impersonate(promoteurMember.user.id, bar.id, 'promoteur'); onClose(); } else { alert('Aucun promoteur trouvé pour ce bar'); } }} className="px-3 py-2 bg-amber-100 text-amber-700 rounded-lg font-semibold text-xs hover:bg-amber-200 flex items-center justify-center gap-1.5" title="Se connecter en tant que promoteur"><UserCog className="w-3.5 h-3.5" />Impersonate</button>
