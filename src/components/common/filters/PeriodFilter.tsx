...
export const PeriodFilter: React.FC<PeriodFilterProps> = ({
    ...
    return (
        <div className={`space-y-3 px-1 ${className}`}>
            ...
            <div className={`flex flex-wrap bg-card/40 dark:bg-muted/40 backdrop-blur-md rounded-2xl p-1 gap-1.5 border border-brand-subtle shadow-sm overflow-hidden sm:w-auto ${justifyClass}`}>
                {availableFilters.map((filter) => (
                    <Button
                        ...
                        className={`px-3 sm:px-4 py-2 h-10 rounded-xl text-[10px] font-black uppercase tracking-tight transition-all flex-1 sm:flex-none sm:min-w-[90px] whitespace-nowrap min-w-max ${timeRange === filter
                            ...
                            : 'glass-action-button-2026 text-muted-foreground hover:text-brand-primary'
                            } ${buttonClassName}`}
                    >
                        {getLabel(filter)}
                    </Button>
            ...
            {isCustom && customRange && updateCustomRange && (
                <div className="flex items-center gap-2 bg-card/40 dark:bg-muted/40 backdrop-blur-md p-3 rounded-2xl border border-brand-subtle shadow-sm animate-in fade-in slide-in-from-top-1">
                    <Input
                        type="date"
                        value={customRange.start}
                        onChange={(e) => updateCustomRange('start', e.target.value)}
                        className="flex-1 text-sm h-9 bg-card rounded-lg border-brand-subtle focus:ring-brand-primary focus:border-brand-primary transition-all"
                    />
