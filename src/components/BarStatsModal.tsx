...
type CombinedStats = {
  ...
  top_products_json: { product_id: string; name: string; rank: number; revenue: number; quantity: number }[];
  // Live
    ...
    const loadAllStats = async () => {
      ...
      try {
        const [financialStats, ancillaryStats, liveAlerts] = await Promise.all([
          ...
          supabase.from('bar_ancillary_stats').select('*').eq('bar_id', bar.id).single(),
