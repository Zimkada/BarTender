    ...
    return (
        <div className="space-y-8 p-6 sm:p-10 bg-slate-50/30 animate-pulse">
            ...
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
                ...
                <div className="w-full lg:w-64 h-12 bg-muted rounded-2xl" />
            </div>

            {/* KPI Grid Skeleton */}
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {Array.from({ length: 4 }).map((_, i) => (
                    <div key={i} className="bg-card p-6 rounded-[2rem] border border-border h-44 flex flex-col justify-between">
                        <div className="flex justify-between items-start">
                            <div className="w-12 h-12 bg-muted rounded-2xl" />
                            <div className="w-16 h-6 bg-muted rounded-xl" />
                        </div>
                        <div className="space-y-2">
            ...
            <div className="bg-card rounded-[2.5rem] border border-border h-[500px] overflow-hidden">
                ...
                <div className="p-6 space-y-4">
                    {Array.from({ length: 5 }).map((_, i) => (
                        <div key={i} className="h-16 bg-muted rounded-2xl w-full" />
                    ))}
                </div>
            </div>
        </div>
